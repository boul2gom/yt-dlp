...
fn handle_event(state: &mut StatsInner, event: &DownloadEvent) {
    match event {
        ...
        DownloadEvent::DownloadCanceled { download_id, .. } => {
            ...
            tracing::debug!(download_id = download_id, "Statistics: download canceled");
        }
        ...
        DownloadEvent::PlaylistFetchFailed { url, duration, .. } => {
            ...
            tracing::debug!(
                url = url,
                duration = ?duration,
                "Statistics: playlist fetch failed"
            );
        }
        ...
        DownloadEvent::PostProcessCompleted { duration, .. } => {
